...
def mcp_list() -> None:
    ...
    for name, entry in mcp_servers.items():
        ...
        cmd_display = f"{cmd} {args_str}".rstrip() if args_str else cmd
        click.echo(f"    [{name}] {cmd_display}")
        if desc:
            click.echo(f"           {desc}")
...
@click.option("--editor", "-e", type=click.Choice(["claude-code", "codebuddy", "all"]),
...
def mcp_export(editor: str) -> None:
    ...
    editors = ["claude-code", "codebuddy"] if editor == "all" else [editor]
...
def _export_to_editor(mcp_servers: dict, editor: str) -> None:
    ...
    if editor == "claude-code":
        ...
        click.echo("  Exported to Claude Code")
        ...
        click.echo("  Exported to CodeBuddy")
...
def _resolve_skill_path(name: str, project: bool) -> Path:
    """Resolve the base directory for a skill."""
    if project:
        return Path(".pygitnexus") / "skills" / name
    return SKILLS_DIR / name


def _skill_base(project: bool) -> Path:
    """Resolve the skills root directory."""
    return Path(".pygitnexus") / "skills" if project else SKILLS_DIR
...
@click.option("--force", "-f", is_flag=True, default=False, help="Overwrite existing SKILL.md")
def skill_init(name: str, description: str, project: bool, force: bool) -> None:
    ...
    if "/" in name or "\\" in name:
        click.echo(f"  Error: Skill name cannot contain '/' or '\\\\'.")
        return

    base = _resolve_skill_path(name, project)
    if (base / "SKILL.md").exists() and not force:
        click.echo(f"  Error: Skill '{name}' already exists at: {base}")
        click.echo(f"  Use '--force' to overwrite, or edit SKILL.md manually.")
        return
...
Add executable scripts or reference docs in `scripts/` directory.
...
def skill_remove(name: str, project: bool) -> None:
    ...
    try:
        ...
        click.echo(f"  Error: Invalid skill path: {skill_dir}")
        return
...
def skill_show(name: str, project: bool) -> None:
    ...
    # Limit output to avoid terminal flooding
    lines = content.splitlines()
    if len(lines) > 100:
        for line in lines[:100]:
            click.echo(f"  {line}")
        click.echo(f"  ... ({len(lines) - 100} more lines, see file directly)")
    else:
        for line in lines:
            click.echo(f"  {line}")
    click.echo("  " + "-" * 40)
...
def _setup_hooks(hook_dir: Path, settings_path: Path, bin_path: str,
                 result: dict, editor_label: str) -> None:
    """Extract hook script and configure PreToolUse/PostToolUse hooks."""
    ...
    if not hook_script.is_file():
        try:
            ...
            result["errors"].append(f"{editor_label} hook extract: {e}")
            return
    ...
    try:
        ...
        if settings_path.is_file():
            ...
            if settings_data is None:
                result["errors"].append(f"{editor_label} hooks: {settings_path} is not valid JSON, skipping")
                return
        else:
            settings_data = {}
        ...
        result["configured"].append(f"{editor_label} (hooks)")
    except Exception as e:
        result["errors"].append(f"{editor_label} hooks: {e}")
...
def _setup_claude_code(result: dict, bin_path: str) -> None:
    ...
    settings_path = Path.home() / ".claude" / "settings.json"
    _setup_hooks(hook_dir, settings_path, bin_path, result, "Claude Code")
...
def _setup_codebuddy(result: dict, bin_path: str) -> None:
    ...
    _setup_hooks(hook_dir, settings_path, bin_path, result, "CodeBuddy")
...
def setup_cmd(ctx, project: bool) -> None:
    ...
    Without subcommand: auto-detect and configure MCP for Claude Code / CodeBuddy.
    With --project: also write <cwd>/.claude/settings.json and <cwd>/.codebuddy/mcp.json for project-level MCP.
...
def setup_auto(project: bool) -> None:
    """Auto-detect and configure MCP for Claude Code / CodeBuddy."""
    click.echo("")
    ...
    # Project-level: write <cwd>/.claude/settings.json and <cwd>/.codebuddy/mcp.json
    if project:
